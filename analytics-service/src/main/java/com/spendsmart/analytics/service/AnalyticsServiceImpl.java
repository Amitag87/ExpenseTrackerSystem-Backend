...
public class AnalyticsServiceImpl implements AnalyticsService {
    ...
    public FinancialSnapshot generateMonthlySnapshot(Long userId, int year, int month) {
        ...
        double income = dataClient.getMonthlyIncome(userId, year, month);
        double expense = dataClient.getMonthlyExpenses(userId, year, month);

        double savings = income - expense;
        double savingsRate = (income == 0) ? 0 : (savings / income) * 100;
        String topCategory = dataClient.getTopExpenseCategory(userId, year, month);

        FinancialSnapshot snapshot = repo.findByUserIdAndYearAndMonth(userId, year, month)
        ...
        snapshot.setTotalExpenses(expense);
        snapshot.setNetSavings(savings);
    ...
    public Map<String, Double> getExpenseBreakdownByCategory(Long userId) {
        Map<String, Double> liveBreakdown = dataClient.getExpenseBreakdownByCategory(userId);
        if (!liveBreakdown.isEmpty()) {
    ...
    public Map<String, Double> getCashflowData(Long userId) {
        List<FinancialSnapshot> snapshots = repo.findByUserId(userId);
        return Map.of(
                "inflow", snapshots.stream().mapToDouble(FinancialSnapshot::getTotalIncome).sum(),
                "outflow", snapshots.stream().mapToDouble(FinancialSnapshot::getTotalExpenses).sum()
        );
    }
